...
const Carousel: React.FC<CarouselProps> = ({ children, className = "", scrollStep }) => {
  ...
  return (
    <div className={`relative w-full h-full ${className}`}>
      {/* bouton gauche */}
      {showLeft && (
        <div className="z-10 absolute left-0 bg-gradient-to-r from-white rounded-xl to-transparent h-full grid place-items-center px-2">
          <button
            onClick={() => scroll("left")}
      ...
      {/* bouton droit */}
      {showRight && (
        <div className="z-10 absolute right-0 top-0 bg-gradient-to-l rounded-xl from-white via-5% to-transparent h-full grid place-items-center px-2">
          <button
